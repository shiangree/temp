...
public class LogParserParser {
    ...
    private ArrayList<Dependency> deps = new ArrayList<Dependency>();
     ...
     */
    @Deprecated
    public LogParserResult parseLog(final AbstractBuild<?, ?> build) throws Exception {
        return this.parseLog((Run<?, ?>) build);
    }

    public LogParserResult parseLog(final Run<?, ?> build) throws Exception {
    ...
    private void parseLogBody(final Run<?, ?> build, final BufferedWriter writer, final FilePath filePath, final
            String logFileLocation, final int linesInLog, final Logger logger) throws Exception {
 ...
 + " minutes ("
    ...
    private ArrayList<Dependency> getpom(Run<?,?> build) throws Exception{
	String buildpath = build.getLogFile().getAbsolutePath();
	int idx1 = buildpath.lastIndexOf("build");
        String dir = buildpath.substring(0,idx1);
        dir = dir+"workspace";
        buildpath = buildpath.substring(0,buildpath.length()-3);
        ArrayList<String> alist = new ArrayList<String>();
        RootPomDiff.findFiles(dir,alist);
        dir = alist.get(0);
	return RootPomDiff.parsePom(dir);
    }
}
